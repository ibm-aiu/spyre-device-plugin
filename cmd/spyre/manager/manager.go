...
func (rm *ResourceManager) ReadConfig() error {
	...
	for i := range resources.ResourceList {
		...
		disableVfMode := os.Getenv(spyrev1alpha1.DisableVfMode.EnvKey()) == spyreconst.ModeEnabledValue
		if disableVfMode && conf.ResourceName == spyreconst.VfResourceName {
			continue
		}
		// Validate deviceType
		if conf.DeviceType == "" {
...
func createResourceServer(
	rf types.ResourceFactory, rc *types.ResourceConfig, devices []types.PciDevice) (
	...
	for _, rc := range rm.configList {
		...
		if os.Getenv(spyrev1alpha1.PerDeviceAllocationMode.EnvKey()) == spyreconst.ModeEnabledValue &&
			...
			for _, dev := range filteredDevices {
				...
				isPfResource := rc.ResourceName == spyreconst.PfResourceName
				isIsolatedVf := rc.ResourceName == spyreconst.VfResourceName && dev.IsIsolatedVF()
				if isPfResource || isIsolatedVf {
...
func (rm *ResourceManager) HandleHotpluggedDevices(
	ctx context.Context, cfg *rest.Config, spyreClient *spyreclient.SpyreClient,
	newDevices []types.PciDevice) {
	glog.Infof("Hotplug event: %d new devices detected, adding per-device servers "+
		"and triggering resource updates", len(newDevices))
...
func (rm *ResourceManager) addPerDeviceServersForNewDevices(newDevices []types.PciDevice) {
	...
	for _, newDevice := range newDevices {
		...
		var pciAddress string
		isPfResource := resourceName == spyreconst.PfResourceName
		isIsolatedVf := resourceName == spyreconst.VfResourceName && newDevice.IsIsolatedVF()
		if isPfResource || isIsolatedVf {
...
func (rm *ResourceManager) updateAggregatedResourcePools(newDevices []types.PciDevice) {
	...
	for i, rs := range rm.resourceServers {
		...
		case perDeviceMode && strings.HasPrefix(resourceName, spyreconst.PfResourceName+"_") &&
			isPf && !strings.HasSuffix(resourceName, "tier"):
			shouldUpdate = true
			configName = spyreconst.PfResourceName // base PF config
		case perDeviceMode && strings.HasPrefix(resourceName, spyreconst.VfResourceName+"_") &&
			isVf && !strings.HasSuffix(resourceName, "tier"):
			shouldUpdate = true
		...
		if perDeviceMode &&
			...
			glog.V(1).Infof("Per-device filtering for %s: extracted PCI addr '%s' from %d devices",
				resourceName, pciAddr, len(allDevices))
...
func (rm *ResourceManager) excludeAllocatedDevices(
	filteredDevices []types.PciDevice, deviceAllocated map[string]bool) []types.PciDevice {
